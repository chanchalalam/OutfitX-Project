...
def is_clothing_image(image_path):
    """Basic check to filter out non-clothing images using image mode & size"""
    try:
        img = Image.open(image_path)
        
        # Reject images that are too small (e.g., icons, logos)
        if img.size[0] < 100 or img.size[1] < 100:
            return False
        
        # Convert image to grayscale and check pixel variance
        grayscale_img = img.convert("L")
        variance = sum(grayscale_img.histogram()) / len(grayscale_img.histogram())
        
        # If the variance is too low, the image might be a blank/solid color
        return variance > 10  

    except Exception:
        return False  # If image processing fails, reject it
...
def single_classification(image_path):
    model_path = "model details/models/model_sub"  # Ensure this is correct
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model path does not exist: {model_path}")

    print(f"Loading model from: {model_path}")
    model = load_model(model_path)  # Make sure model loads correctly
    ...
    prediction = model.predict(img_array)  # Ensure this is the correct call
    predicted_class = np.argmax(prediction)  # Convert to label

    # Dummy class mapping
    class_labels = {0: "top", 1: "bottom", 2: "foot"}
    category = class_labels.get(predicted_class, "unknown")

    return category, f"Predicted: {category}", ["Casual", "Summer", "Trendy"]
